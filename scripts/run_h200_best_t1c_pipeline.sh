...
export STEP_CHECKPOINT_EVERY="${STEP_CHECKPOINT_EVERY:-500}"
export CHECKPOINT_KEEP_LAST="${CHECKPOINT_KEEP_LAST:-3}"
...
export STOCHASTIC_INITIAL_STATE="${STOCHASTIC_INITIAL_STATE:-0}"
export STOCHASTIC_NOISE_SCALE="${STOCHASTIC_NOISE_SCALE:-0.05}"
export POSTERIOR_SAMPLES="${POSTERIOR_SAMPLES:-1}"
export RUN_FULL_VOLUME_EVAL="${RUN_FULL_VOLUME_EVAL:-1}"
export FULL_VOLUME_BATCH_SLICES="${FULL_VOLUME_BATCH_SLICES:-32}"
export RUN_FUSION_EVAL="${RUN_FUSION_EVAL:-0}"
export FUSION_CHECKPOINT="${FUSION_CHECKPOINT:-}"
export BRAINMVP_CHECKPOINT="${BRAINMVP_CHECKPOINT:-${ROOT}/pretrained/BrainMVP_uniformer.pt}"
export FUSION_SPATIAL_SIZE="${FUSION_SPATIAL_SIZE:-96}"
...
if [[ "${TRAIN_STAGE2_HARD}" == "1" ]]; then
  ...
  echo "[H200] Best checkpoint: ${OUTPUT_ROOT}/${RUN_NAME}_transport/slice_virtual_modality_generator_best.pt"
fi
